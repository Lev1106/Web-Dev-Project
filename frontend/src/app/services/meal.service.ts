...
export class MealService {
  ...
  getGoal() {
    return this.http.get<Goal>(`${this.apiUrl}/goal/`).pipe(
        catchError(err => {
          this.errorMsg.set('Не удалось загрузить цель по калориям');
          return throwError(() => err);
        })
    );
  }
  ...
  getTodayMeals() {
    return this.http.get<Meal[]>(`${this.apiUrl}/meals/?today=true`).pipe(
        catchError(err => {
          this.errorMsg.set('Не удалось загрузить сегодняшние приёмы пищи');
          return throwError(() => err);
        })
    );
  }
  ...
  todayTotal = computed(() =>
    this.meals().reduce((sum, m) => sum + m.calories, 0)
  );
  ...
  avgPerMeal = computed(() => {
    const m = this.meals();
    return m.length ? Math.round(m.reduce((s, x) => s + x.calories, 0) / m.length) : 0;
  });
  ...
  deleteMeal(id: number) {
    ...
    return this.http.delete(`${this.apiUrl}/meals/${id}/`).pipe(
      catchError(err => {
        this.errorMsg.set('Не удалось удалить приём пищи');
        return throwError(() => err);
      })
    );
  }
  ...
  allHistory = computed(() => {
    ...
    this.historyMeals().forEach(m => {
      const label = new Date(m.eaten_at).toLocaleDateString([], { month: 'short', day: 'numeric' });;
    ...
    return Object.entries(grouped).map(([date, meals]) => ({
      date,
      meals,
      total: meals.reduce((s, m) => s + m.calories, 0)
    }));
  });
}
